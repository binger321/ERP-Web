...
// 在这里引入了config.js
  ...
  $("#add_submit").click(function(event) {
    ...
    $.ajax({
      url: host + 'erp-svc-goods/goodsBrand/add',
      type: 'post',
      dataType: 'json',
      contentType: 'application/json; charset=utf-8',
      data: tableForm,
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $.ajax({
            type: "post",
            url: host + 'erp-svc-goods/goodsBrand/delete/' + deleteId,
            beforeSend: function(request) {
              request.setRequestHeader("Authorization", "Bearer "+token);
            },
  ...
  $("#updateBtn").click(function(event) {
    ...
    $.ajax({
      type: 'post',
      contentType: 'application/json; charset=utf-8',
      url: host + "erp-svc-goods/goodsBrand/" + deleteId,
      beforeSend: function(request) {
        request.setRequestHeader("Authorization", 'Bearer ' + token);  
      },
      success: function(result) {
        if (result.status == 0) {
          ...
          $("#brandCode").val(resultData.brandCode);
          $("#brandName").val(resultData.brandName);
          $("#countryName").val(resultData.countryName)
  ...
  $('#update_submit').click(function(event) {
    ...
    $.ajax({
      type: 'post',
      contentType: 'application/json; charset=utf-8',
      url: host + "erp-svc-goods/goodsBrand/update/" + deleteId,
      data: updateForm,
      beforeSend: function(request) {
        request.setRequestHeader("Authorization", 'Bearer ' + token);  
      },
