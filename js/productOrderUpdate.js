...
// 在这里引入了config.js
  ...
  $(function(){
    ...
    $("#productOrderCode").val(dataObject.productOrderCode);
    $("#auditTime").val(dataObject.auditTime);
    $("#auditUserName").val(dataObject.auditUserName);
    $("#localTotalMny").val(dataObject.localTotalMny);
    $("#quantity").val(dataObject.quantity);
    $("#weight").val(dataObject.weight);
    $("#supplierId").val(dataObject.supplierId);
    $("#logistCompanyName").val(dataObject.logistCompanyName);
    $("#warehouseId").val(dataObject.warehouseId);
    $("#warehouseCode").val(dataObject.warehouseCode);
    $("#warehouseName").val(dataObject.warehouseName);
    $("#orderNumber").val(dataObject.orderNumber);
    $("#billDate").val(dataObject.billDate);
    $("#orderStatus").val(dataObject.orderStatus);
 ...
 /*保存数据*/
  $("#saveBtn").click(function() {
    ...
    $.ajax({
      type: 'post',
      contentType: 'application/json; charset=utf-8',
      url: stockHost + "erp-svc-stock/productOrder/orderMain/update/"+dataObject.id,
 ...
 $("#showBtn").click(function(event) {
  ...
  $.ajax({
    type: 'post',
    contentType: 'application/json; charset=utf-8',
    url: stockHost + "erp-svc-stock/productOrder/orderDetail/findAllOrderDetailByOrderMainId/"+dataObject.id,
    // +"?pageNo=" +fs._iDisplayStart + "&pageSize=" + fs._iDisplayLength,
    data: lastQuery,
    beforeSend: function(request) {
      request.setRequestHeader("Authorization", "Bearer " + token);
  ...
  $("#add_submit").click(function(event) {
    ...
    form.productOrderMainCode = sku.productOrderMainCode;
    ...
    form.productOrderMainId = dataObject.id;
    var tableForm = JSON.stringify(form);
    $.ajax({
      url : stockHost+'erp-svc-stock/productOrder/orderDetail/insert',
      type: 'post',
      dataType: 'json',
      contentType: 'application/json; charset=utf-8',
      data: tableForm,
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $.ajax({
            type: "post",
            url: stockHost + 'erp-svc-stock/productOrder/orderDetail/delete/' + deleteId,
            beforeSend: function(request) {
              request.setRequestHeader("Authorization", "Bearer "+token);
            },
...
/*修改数据*/
  ...
  $("#updateBtn").click(function(event) {
    ...
    $.ajax({
      type: 'post',
      contentType: 'application/json; charset=utf-8',
      url: stockHost + "erp-svc-stock/productOrder/orderDetail/findById/" + deleteId,
      beforeSend: function(request) {
        request.setRequestHeader("Authorization", 'Bearer ' + token);  
      },
      success: function(result) {
        if (result.status == 0) {
          ...
          $("#lastPrice").val(result.data.lastPrice);
          $("#localPrice").val(result.data.localPrice);
          $("#isStorage").val(result.data.isStorage);
          $("#totalQuantity").val(result.data.totalQuantity);
  ...
  $('#update_submit').click(function(event) {
    ...
    $.ajax({
      type: 'post',
      contentType: 'application/json; charset=utf-8',
      url: stockHost + "erp-svc-stock/productOrder/orderDetail/update/" + deleteId,
      data: updateForm,
      beforeSend: function(request) {
        request.setRequestHeader("Authorization", 'Bearer ' + token);  
      },
