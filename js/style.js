...
// 在这里引入了config.js
  ...
  $("#add_submit").click(function(event) {
    ...
    $.ajax({
      url: host + 'erp-svc-goods/goodsStyle/add',
      type: 'post',
      dataType: 'json',
      contentType: 'application/json; charset=utf-8',
      data: tableForm,
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $.ajax({
            type: "post",
            url: host + 'erp-svc-goods/goodsStyle/delete/' + deleteId,
            beforeSend: function(request) {
              request.setRequestHeader("Authorization", "Bearer "+token);
            },
  ...
  $("#updateBtn").click(function(event) {
    ...
    $.ajax({
      type: 'post',
      contentType: 'application/json; charset=utf-8',
      url: host + "erp-svc-goods/goodsStyle/" + deleteId,
      beforeSend: function(request) {
        request.setRequestHeader("Authorization", 'Bearer ' + token);  
      },
      success: function(result) {
        if (result.status == 0) {
          ...
          $("#styleCode").val(resultData.styleCode);
          $("#styleName").val(resultData.styleName);
  ...
  $('#update_submit').click(function(event) {
    ...
    $.ajax({
      type: 'post',
      contentType: 'application/json; charset=utf-8',
      url: host + "erp-svc-goods/goodsStyle/update/" + deleteId,
      data: updateForm,
      beforeSend: function(request) {
        request.setRequestHeader("Authorization", 'Bearer ' + token);  
      },
