...
// 在这里引入了config.js
  ...
  $("#selectBtn").click(function() {
    ...
    $.ajax({
      type: 'post',
      contentType: 'application/json; charset=utf-8',
      url: host + "erp-svc-stock/productOrder/list",
  ...
  $('#dataTable tbody').on( 'click', 'button#editrow', function () {
    ...
    var url='productOrderDetali.html';                             //转向网页的地址; 
    ...
    w=window.open(url, name, 'height=' + iHeight + ',,innerHeight=' + iHeight + ',width=' + iWidth + ',innerWidth=' + iWidth + ',top=' + iTop + ',left=' + iLeft + ',status=no,toolbar=no,menubar=no,location=no,resizable=no,scrollbars=0,titlebar=no');     
  ...
  $("#add_submit").click(function(event) {
    ...
    $.ajax({
      url: stockHost + 'erp-svc-stock/productOrder/orderMain/insert',
      type: 'post',
      dataType: 'json',
      contentType: 'application/json; charset=utf-8',
      data: tableForm,
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $.ajax({
            type: "post",
            url: stockHost + 'erp-svc-stock/productOrder/orderMain/delete/' + deleteId,
            beforeSend: function(request) {
              request.setRequestHeader("Authorization", "Bearer "+token);
            },
  ...
  $("#updateBtn").click(function(event) {
    ...
    $.ajax({
      type: 'post',
      contentType: 'application/json; charset=utf-8',
      url: stockHost + "erp-svc-stock/productOrder/orderMain/findById/" + deleteId,
      beforeSend: function(request) {
        request.setRequestHeader("Authorization", 'Bearer ' + token);  
      },
      success: function(result) {
        ...
        window.location.href = 'productOrderUpdate.html'; 
      },
      error: function() {
        toastr.error('获取订单失败！');
      }
