...
// 在这里引入了config.js
  ...
  $(function(){
    ...
    $("#orderCode").val(dataObject.orderCode);
    $("#countryName").val(dataObject.countryName);
    $("#state").val(dataObject.state);
    $("#city").val(dataObject.city);
    $("#address1").val(dataObject.address1);
    $("#phoneNumber").val(dataObject.phoneNumber);
    $("#customerName").val(dataObject.customerName);
    $("#buyerName").val(dataObject.buyerName);
    $("#customerEmail").val(dataObject.customerEmail);
    $("#logistCompanyName").val(dataObject.logistCompanyName);
    $("#trackNumber").val(dataObject.trackNumber);
    $("#shippingFee").val(dataObject.shippingFee);
    $("#remark1").val(dataObject.remark1);
  ...
  $("#showStore").click(function(){
    ...
    $.ajax({
      ...
      success: function(result) {
        if (result.status == 0) {
          var select = $("#storeId_update");
          select.toggle();
          select.empty();
          var resultList = result.data;
          resultList.forEach(function(ele, index){
            if(resultList[index].id == storeId) {
              select.append("<option value='"+resultList[index].id+"' selected='selected'>"+resultList[index].storeName+"</option>"); 
            } else {
              select.append("<option value='"+resultList[index].id+"'>"+resultList[index].storeName+"</option>"); 
            }
          });
          //初始化刷新数据  
          $("#storeId_update").removeAttr("disabled");
          $("#storeId_update").selectpicker('refresh');
        } else {
          toastr.error('获取店铺失败！');
        }
      },
 ...
 $("#showBtn").click(function(event) {
  ...
  $.ajax({
    type: 'post',
    contentType: 'application/json; charset=utf-8',
    url: host + "erp-svc-goods/saleOrder/findAllDetail/"+dataObject.id,
    // +"?pageNo=" +fs._iDisplayStart + "&pageSize=" + fs._iDisplayLength,
    data: lastQuery,
    beforeSend: function(request) {
      request.setRequestHeader("Authorization", "Bearer " + token);
  ...
  $("#add_submit").click(function(event) {
    ...
    form.goodsId = form.skuId.split("-")[1];
    form.skuId = form.skuId.split("-")[0];
    form.orderSaleId = dataObject.id;
    var tableForm = JSON.stringify(form);
    $.ajax({
      url: host + 'erp-svc-goods/saleOrder/addDetail',
      type: 'post',
      dataType: 'json',
      contentType: 'application/json; charset=utf-8',
      data: tableForm,
    ...
    swal({
      ...
      .then((willDelete) => {
        if (willDelete) {
          $.ajax({
            type: "post",
            url: host + 'erp-svc-goods/saleOrder/deleteDetail/' + deleteId,
            beforeSend: function(request) {
              request.setRequestHeader("Authorization", "Bearer "+token);
            },
...
/*修改数据*/
  ...
  $("#updateBtn").click(function(event) {
    ...
    $.ajax({
      type: 'post',
      contentType: 'application/json; charset=utf-8',
      url: host + "erp-svc-goods/saleOrder/findDetail/" + deleteId,
      beforeSend: function(request) {
        request.setRequestHeader("Authorization", 'Bearer ' + token);  
      },
      success: function(result) {
        if (result.status == 0) {
          ...
          $("#localPrice").val(result.data.localPrice);
          ...
          $.ajax({
            ...
            success: function(result) {
              if (result.status == 0) {
                var select = $("#skuId_update");
                select.toggle();
                select.empty();
                var resultList = result.data;
                resultList.forEach(function(ele, index){
                  if (resultList[index].id == skuId) {
                    select.append("<option value='"+resultList[index].id+"-"+resultList[index].goodsId+"' selected = 'selected'>"+resultList[index].goodsCode+"+"+resultList[index].colorName+"+"+resultList[index].sizeCode+"</option>"); 
                  } else {
                    select.append("<option value='"+resultList[index].id+"-"+resultList[index].goodsId+"'>"+resultList[index].goodsCode+"+"+resultList[index].colorName+"+"+resultList[index].sizeCode+"</option>"); 
                  }
                })
                //初始化刷新数据  
                $("#skuId_update").selectpicker('refresh');
              } else {
                toastr.error('获取SKU失败！');
              }
            },
  ...
  $('#update_submit').click(function(event) {
    ...
    $.ajax({
      type: 'post',
      contentType: 'application/json; charset=utf-8',
      url: host + "erp-svc-goods/saleOrder/updateDetail/" + deleteId,
      data: updateForm,
      beforeSend: function(request) {
        request.setRequestHeader("Authorization", 'Bearer ' + token);  
      },
